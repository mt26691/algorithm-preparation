...
namespace BinaryTree._06.InorderTravesal
{
    class Program
    {
        static void Main(string[] args)
        {
            ...
            InOrderTraverse(binayTree.Root);
            Console.ReadLine();
        }

        public static void InOrderTraverse<T>(BinaryTreeNode<T> rootNode) where T : IComparable<T>
        {
            if(rootNode == null)
            {
                return;
            }
            InOrderTraverse<T>(rootNode.Left);
            Console.WriteLine($"Travel to {rootNode.Value}");
            InOrderTraverse<T>(rootNode.Right);
        }
    }
}
